...
block, or one naming a folder, covers nothing.[^cited] A citation is reported as naming a file that does not
...
| `1` | `coverage.include` matches no file, or a pattern list is not a list of patterns[^exit] | `wiki: wiki.toml: coverage.include matches no files in /path/to/notes; name the project's source files relative to the project, such as include = ["src/**/*.py"]` |
...
[^cited]: `src/builder/build.py` — `cited_paths()` reads the backticked names in footnotes outside fenced
    code; `coverage()` counts a name that is a file, and reports one as missing only when it has an
...
[^exit]: `src/builder/cli.py` — `run()` returns 0 after the report and 2 when `coverage()` returns `None`;
    `main()` returns 1 for a `WikiError`, which `read_coverage()` and `coverage()` in `src/builder/` raise,
