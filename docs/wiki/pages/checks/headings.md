...
group = "Rules"
rows = [
  { label = "Refusals", value = "questions, verdicts, pointers", cite = ["question", "verdict", "pointer"] },
  { label = "Scope", value = "headings from the second level to the sixth", cite = "reach" },
  { label = "Exceptions", value = "a line inside a code sample", cite = "reach" },
]
+++
...
pointing words and vague actors, described on [Wording](wording.md).

## Exceptions

A line inside a code sample is not a heading, and is not checked.[^reach]
