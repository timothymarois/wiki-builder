...
drawer.[^check] The check refuses the two kinds of heading that name nothing.[^check]
...
Every heading from the second level to the sixth is checked, but the page title is not.[^reach] A line
inside a code sample is not a heading, and is not checked.[^reach]
